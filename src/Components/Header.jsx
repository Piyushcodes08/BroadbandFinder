...
const Header = () => {
  ...
  const linkBase =
    "text-gray-900 text-base lg:text-lg font-medium transition-colors hover:text-[#1f6300]";
  const linkActive = "text-[#1f6300]";

  return (
    <div className="relative">
      {/* Topbar */}
      <div className="h-8 bg-[#1f6300] flex items-center justify-between md:justify-end px-4 md:px-16">
        <a
          href="mailto:sales@zenithlink.us"
          className="hidden sm:flex items-center gap-2 text-white hover:text-gray-200 transition"
        >
          <span className="text-sm font-bold">sales@zenithlink.us</span>
        </a>
      ...
      {/* Header */}
      <header
        className={`w-full z-50 border-b border-gray-200 bg-white ${
          isAdminRoute ? "relative" : "sticky top-0"
        }`}
      >
        <div className="flex items-center justify-between py-2 md:px-16 px-4">
          {/* Logo */}
          <Link to="/" className="flex items-center h-14 sm:h-16 shrink-0" aria-label="ZenithLink Home">
            <img
              src={logo}
              alt="ZenithLink Logo"
          ...
          <nav className="hidden lg:flex items-center gap-6">
            ...
            >
              <button
                type="button"
                className={`${linkBase} ${cloudOpen ? "text-[#1f6300]" : ""} border-r pr-4 flex items-center gap-1`}
                aria-haspopup="menu"
            ...
            {isLoggedIn && (
              <>
                ...
                <button
                  ...
                  className="ml-2 px-3 py-1 rounded bg-red-500 text-white shadow hover:bg-red-600 transition"
                >
                  Logout
                </button>
              </>
        ...
        >
          <div className="flex items-center justify-between px-4 py-3 border-b">
            <Link to="/" onClick={() => setMenuOpen(false)} aria-label="ZenithLink Home">
              <img src={logo} alt="ZenithLink Logo" className="h-10 object-contain" />
            </Link>
            <button
              onClick={() => setMenuOpen(false)}
                ...
                <div id="cloud-menu-mobile" role="menu" className="ml-3 mt-1 border-l pl-3">
                  <ul className="space-y-1">
                    <li>
                      <Link
                        to="/internet/cloudsevices/ringcentralvoip"
                        className="block py-2 hover:text-[#1f6300]"
                        role="menuitem"
                        onClick={() => setMenuOpen(false)}
                      >
                        RingCentral
            ...
            {isLoggedIn && (
              <>
                ...
                <button
                  ...
                  className="mt-3 w-full px-3 py-2 rounded bg-red-500 text-white shadow hover:bg-red-600 transition"
                >
                  Logout
                </button>
              </>
