...
const DRAFT_KEY = "draft_att_business_v1";

const PLANS = [
  { title: "300 Mbps", price: 40 },
  { title: "500 Mbps", price: 80 },
  { title: "1 GIG", price: 130 },
  { title: "2 GIG", price: 155 },
  { title: "5 GIG", price: 255 },
];

const INSTALL = [{ id: "pro", name: "Professional Installation", fee: 99 }];

const STATIC_IP_TIERS = [
  { qty: 0, price: 0 },
  { qty: 1, price: 20 },
  { qty: 5, price: 30 },
  { qty: 13, price: 40 },
...
export default function AttBusinessCheckout() {
  ...
  const price = useMemo(() => {
    ...
    // Voice seats (AT&T UC/Voice optional)
    monthly += (cfg.voiceSeats || 0) * 20;

    // Additional telephone lines add-on priced at $15/line for AT&T
    monthly += (cfg.telLinesAddon || 0) * 15;

    // Static IP tiers
    const q = Number(cfg.staticIpQty || 0);
    const tier = STATIC_IP_TIERS.filter((t) => t.qty <= q).pop() || {
      price: 0,
    };
    monthly += tier.price;

    const oneTime = INSTALL.find((i) => i.id === install)?.fee || 0;
    return { monthly, oneTime };
  }, [plan, cfg, install]);

  const addrOk = (a) => a.street && a.city && a.state && a.zip;
  const can1 = Boolean(plan);
    ...
    <div className="max-w-5xl mx-auto p-6" data-aos="fade-up">
      ...
      {step === 1 && (
        <section className="grid gap-6 lg:grid-cols-[1fr_320px]">
          <Card title="Plan & Options (simple)">
            ...
            <div className="mt-6 grid gap-3 md:grid-cols-2">
              <NumberRow
                label="Static IPs"
                value={cfg.staticIpQty}
                onChange={(n) => setCfg({ ...cfg, staticIpQty: n })}
              />
              <NumberRow
                label="Voice seats"
                value={cfg.voiceSeats}
                onChange={(n) => setCfg({ ...cfg, voiceSeats: n })}
              />
              <NumberRow
                label="Telephone lines add-on"
                value={cfg.telLinesAddon}
                onChange={(n) => setCfg({ ...cfg, telLinesAddon: n })}
              />
            </div>
          </Card>
          <Summary
      ...
      {step === 3 && (
        <section className="grid gap-6 lg:grid-cols-[1fr_320px]">
          <Card title="Contact & Addresses">
            ...
            {!contact.billSame && (
              <>
                <h4 className="mt-3 font-semibold">Billing Address</h4>
                <Addr
                  value={contact.bill}
        ...
        <section className="grid gap-6 lg:grid-cols-[1fr_320px]">
          <Card title="Review">
            <ul className="text-sm space-y-1">
              ...
              <li>
                <b>Install:</b> {INSTALL.find((i) => i.id === install)?.name}
              </li>
              <li>
                <b>Static IPs:</b> {cfg.staticIpQty}
              </li>
              <li>
                <b>Voice seats:</b> {cfg.voiceSeats}
              </li>
              <li>
                <b>Telephone lines add-on:</b> {cfg.telLinesAddon}
              </li>
            </ul>
          </Card>
